...
allIntegersInclusive a b = if a<b then [a..b] else [a,(a-1)..b]
...
allIntegersExcludingFirst a b = if a<b then [(a+1)..b] else [(a-1),(a-2)..b]
...
sumDigits n = sum([(read :: String -> Int) [x] | x <- (show n)])

productDigits n = product([(read :: String -> Int) [x] | x <- (show n)])

isRigid n = sumDigits n == productDigits n
...
fizzOrBuzz n f b = if (mod n b == 0) && (mod n f == 0) then "FizzBuzz" else (if mod n b == 0 then "Buzz" else (if mod n f == 0 then "Fizz" else show n))


fizzBuzz n f b = [fizzOrBuzz x f b | x <- [1..n]]
