...
windChillRisk t v 
  | windChill >=  16 =  "Low Risk"
  | windChill >= (-17) =  "Moderate Risk"
  | windChill >= (-38) =  "High Risk"
  | windChill >= (-53) = "Very High Risk"
  | windChill >= (-65) = "Severe Risk"
  | otherwise = "Extreme Risk"
  where windChill = 35.74 + 0.6215*t - 35.75*v**0.16 + 0.4275*t*v**0.16
...
numListToNum [] = 0
numListToNum nums = ((nums !! 0)*10^(length(nums)-1)) + numListToNum (tail(nums))
